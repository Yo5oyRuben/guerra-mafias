...
if ($OutRoot -eq '') {
  ...
  $betaTag = ([string]::Format($InvariantCulture, '{0:g}', $FermiBeta)).Replace('.', 'p')
  $OutRoot = "c_paper/out/experiments/sweep_b_fig4_fermi_beta_${betaTag}_$stamp"
}
...
if (-not $Controller) {
  ...
  $scriptPath = Join-Path $PSScriptRoot 'run_sweep_b_fig4_fermi_queue.ps1'
  $argsList = ('-NoProfile -ExecutionPolicy Bypass -Command "& ''{0}'' -Controller -OutRoot ''{1}'' ' +
               '-MaxParallel {2} -Reps {3} -NDIV {4} -T_MCS {5} -T_MAX {6} -W {7} -FermiBeta {8}"') -f `
              $scriptPath,$OutRoot,$MaxParallel,$Reps,$NDIV,$T_MCS,$T_MAX,$W,([string]::Format($InvariantCulture, '{0:g}', $FermiBeta))
...
function Build-SweepStats {
  ...
  $flags = @(
    ...
    '-DE=-0.4',
    '-DUPDATE_RULE=1',
    "-DFERMI_BETA=$([string]::Format($InvariantCulture, '{0:g}', $FermiBeta))",
    '-I', $base,
    '-I', "$base/core",
    '-I', "$base/apps",
    '-I', "$base/apps/sweep_b"
...
function Add-JobRow {
  ...
  $out = Join-Path $RawDir ("fig4_fermi_beta_{0}_p12_{1}.tsv" -f ([string]::Format($InvariantCulture, '{0:g}', $FermiBeta)).Replace('.', 'p'),$tagP)
  $log = Join-Path $LogDir ("fig4_fermi_beta_{0}_p12_{1}.log" -f ([string]::Format($InvariantCulture, '{0:g}', $FermiBeta)).Replace('.', 'p'),$tagP)
...
("{0}`tstarting fig4 fermi sweep; beta={1}; independence=graph_per_b_rep" -f (Get-Date -Format s),(Format-Double $FermiBeta)) |
  Add-Content -Path $ControllerLog

$exeFig4 = Join-Path $BuildDir 'sweep_stats_fig4_fermi_N1000_N100.exe'
Build-SweepStats -Exe $exeFig4

$rows = [System.Collections.ArrayList]::new()
$paperP = @(0.0,0.001,0.002,0.005,0.01,0.02,0.04)
$idx = 0
foreach ($p12 in $paperP) {
  Add-JobRow -Rows $rows -Exe $exeFig4 -P12 $p12 -Seed ([uint32](520000 + $idx))
  $idx++
}
