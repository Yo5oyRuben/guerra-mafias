param(
  ...
  [int]$NDiv = 8,
  [int]$NReps = 3,
  [string]$Experiment = '2026-05-06_relaxation_p073_TMAX400k'
)
...
$before = @{}
Get-ChildItem -LiteralPath $LogDir -Filter 'initial_plane_queue_relaxationP073T400k_*.tsv' -ErrorAction SilentlyContinue |
...
& powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run_initial_plane_queue.ps1 `
  -Preset relaxationP073T400k -Jobs $Jobs -NDiv $NDiv -NReps $NReps -NoPlot

if ($LASTEXITCODE -ne 0) {
  throw "La cola relaxationP073T400k fallo con codigo $LASTEXITCODE"
}

$manifest = Get-ChildItem -LiteralPath $LogDir -Filter 'initial_plane_queue_relaxationP073T400k_*.tsv' |
...
if (-not $manifest) {
  throw 'No encuentro el manifest relaxationP073T400k despues de la ejecucion.'
}
...
$notes = @"
# Relaxation P073 TMAX=400000

Pregunta:
Comprobar si el caso delicado de alta conectividad P11=P22=0.73 estabiliza al duplicar T_MAX de 200000 a 400000.
...
$plotPath = Join-Path $PlotsDir 'conn_p073_TMAX_400000.png'
& gnuplot -e "tmax=400000; infile='$($rawPath.Replace('\','/'))'; outfile='$($plotPath.Replace('\','/'))'" "$Base/out/gp/plot_initial_plane.gp"
if ($LASTEXITCODE -ne 0) {
