...
if ($OutRoot -eq '') {
  ...
  $OutRoot = "c_paper/out/experiments/sweep_b_fig3_fermi_beta_${betaTag}_$stamp"
}
...
if (-not $Controller) {
  ...
  $scriptPath = Join-Path $PSScriptRoot 'run_sweep_b_fig3_fermi_queue.ps1'
  $argsList = ('-NoProfile -ExecutionPolicy Bypass -Command "& ''{0}'' -Controller -OutRoot ''{1}'' ' +
               '-MaxParallel {2} -Reps {3} -NDIV {4} -T_MCS {5} -T_MAX {6} -W {7} -FermiBeta {8} -P12List ''{9}''"') -f `
              $scriptPath,$OutRoot,$MaxParallel,$Reps,$NDIV,$T_MCS,$T_MAX,$W,(Format-Double $FermiBeta),$P12List
...
function Build-SweepStats {
  ...
  $flags = @(
    ...
    '-DE=-0.4',
    '-DUPDATE_RULE=1',
    "-DFERMI_BETA=$(Format-Double $FermiBeta)",
    '-I', $base,
    '-I', "$base/core",
    '-I', "$base/apps",
    '-I', "$base/apps/sweep_b"
...
function Add-JobRow {
  ...
  $betaTag = ([string]::Format($InvariantCulture, '{0:g}', $FermiBeta)).Replace('.', 'p')
  $out = Join-Path $RawDir ("fig3_fermi_beta_{0}_p12_{1}.tsv" -f $betaTag,$tagP)
  $log = Join-Path $LogDir ("fig3_fermi_beta_{0}_p12_{1}.log" -f $betaTag,$tagP)
...
("{0}`tstarting fig3 fermi sweep; beta={1}; independence=graph_per_b_rep" -f (Get-Date -Format s),(Format-Double $FermiBeta)) |
  Add-Content -Path $ControllerLog

$exeFig3 = Join-Path $BuildDir 'sweep_stats_fig3_fermi_N1000_N1000.exe'
...
foreach ($p12 in $paperP) {
  Add-JobRow -Rows $rows -Exe $exeFig3 -P12 $p12 -Seed ([uint32](620000 + $idx))
  $idx++
}
