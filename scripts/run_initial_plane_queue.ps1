...
function Format-Invariant([double]$Value) {
  return [string]::Format([Globalization.CultureInfo]::InvariantCulture, "{0:G17}", $Value)
}
...
function Get-RunTag($Job) {
  ...
  $tagParts = @(
    ...
    "rule_$(if ($UpdateRule -eq 1) { 'fermi' } elseif ($UpdateRule -eq 2) { 'degree' } else { 'linear' })",
    "beta_$(Safe-Tag (Format-Invariant $FermiBeta))",
    "alpha_$(Safe-Tag (Format-Invariant $DegreeAlpha))",
    "clamp_$(Safe-Tag (Format-Invariant $DegreeFactorMin))_$(Safe-Tag (Format-Invariant $DegreeFactorMax))",
    "ndiv_$NDiv",
    "reps_$NReps"
  )
...
} elseif ($Preset -eq 'degreeThermoTargeted') {
  $JobsToRun += @(
    [pscustomobject]@{N1='800'; N2='800'; P11='0.9'; P12='0.05'; P22='0.9'; B='1.02'; R='0'; E='-0.4'; T_MCS='1000'; T_MAX='25000'; W='2000'},
    [pscustomobject]@{N1='800'; N2='300'; P11='0.9'; P12='0.02'; P22='0.9'; B='1.06'; R='0'; E='-0.4'; T_MCS='1000'; T_MAX='25000'; W='2000'},
    [pscustomobject]@{N1='500'; N2='500'; P11='0.9'; P12='0.3'; P22='0.9'; B='1.3'; R='0'; E='-0.4'; T_MCS='1000'; T_MAX='50000'; W='2000'}
  )
...
"index`tN1`tN2`tP11`tP12`tP22`tB`tR`tE`tupdate_rule`tfermi_beta`tdegree_alpha`tdegree_factor_min`tdegree_factor_max`tndiv`treps`toutput`tplot" |
...
for ($i = 0; $i -lt $JobsToRun.Count; $i++) {
  ...
  "{0}`t{1}`t{2}`t{3}`t{4}`t{5}`t{6}`t{7}`t{8}`t{9}`t{10}`t{11}`t{12}`t{13}`t{14}`t{15}`t{16}`t{17}" -f `
    ($i + 1), $n1, $n2, $job.P11, $job.P12, $job.P22, $job.B, $job.R, $job.E, $UpdateRule, (Format-Invariant $FermiBeta), (Format-Invariant $DegreeAlpha), (Format-Invariant $DegreeFactorMin), (Format-Invariant $DegreeFactorMax), $NDiv, $NReps, $output, $plot |
    Add-Content -Encoding ASCII -LiteralPath $ManifestPath
}

Write-Log "Cola initial_plane preparada: preset=$Preset simulaciones=$($JobsToRun.Count). Jobs=$Jobs NDiv=$NDiv NReps=$NReps UpdateRule=$UpdateRule FermiBeta=$(Format-Invariant $FermiBeta) DegreeAlpha=$(Format-Invariant $DegreeAlpha) Clamp=[$(Format-Invariant $DegreeFactorMin),$(Format-Invariant $DegreeFactorMax)]"
...
try {
  for ($i = 0; $i -lt $JobsToRun.Count; $i++) {
    ...
    $cfg = Set-DefineValue $cfg 'FERMI_BETA' (Format-Invariant $FermiBeta)
    $cfg = Set-DefineValue $cfg 'DEGREE_ALPHA' (Format-Invariant $DegreeAlpha)
    $cfg = Set-DefineValue $cfg 'DEGREE_FACTOR_MIN' (Format-Invariant $DegreeFactorMin)
    $cfg = Set-DefineValue $cfg 'DEGREE_FACTOR_MAX' (Format-Invariant $DegreeFactorMax)
